...
export const getLoggedUserCart = asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if(!cart){
        return next(new ApiErrors("this user doesnt have carts yet",404))
    };
...
export const upadteProductQuantity = asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const cart: any = await cartsModel.findOne({user: req.user?._id})
    if (!cart) {
        return next(new ApiErrors("this user doesnt have cart yet",404))
    }
    const productIndex = cart.cartItems.findIndex((item: CartProducts) => item._id!.toString() === req.params.itemId.toString());    calcTotalPrice(cart);
    if (productIndex > -1){
